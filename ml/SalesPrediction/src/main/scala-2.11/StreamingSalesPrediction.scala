  ...
  def startRealTimePrediction(spark: SparkSession, HDFS: String): Unit = {
    ...
    lines.foreachRDD { (rdd: RDD[String], time: Time) =>
      ...
      predictions
        ...
        .map(row => (row.getLong(0), row.getString(1), CrossValidatorModelTrainer.sigmoid(row.getAs[org.apache.spark.ml.linalg.DenseVector](2).values(0))))
     ...
     //send result to SQS queue
      if (!predictions.rdd.isEmpty()) {
        ...
        val result = predictions
          .select("AutoLeadID", "predictedLabel", "probability")
          .map(row => (row.getLong(0), row.getString(1), CrossValidatorModelTrainer.sigmoid(row.getAs[org.apache.spark.ml.linalg.DenseVector](2).values(1))))
          .withColumnRenamed("_1", "AutoLeadID")
          .withColumnRenamed("_2", "Predicted")
          .withColumnRenamed("_3", "Confidence")
          .rdd
